...
void PrintAllDependencies(const std::filesystem::path& filePath) {
    try {
        ...
        // Delay-load импорты
        if (const auto delayImports = pe.GetDelayImport(); delayImports.has_value()) {
            std::wcout << L"\n[DELAY-LOAD IMPORTS]\n";
        ...
        if (const auto boundImports = pe.GetBoundImport(); boundImports.has_value()) {
            std::wcout << L"\n[BOUND IMPORTS]\n";
...
int main(int argc, char* argv[]) {
    ...
    std::queue<std::string> file_names_queue;
    std::queue<std::string> file_names_queue_tmp;
    ...
    Directory dir("path");
    std::vector<std::string> no_finded;
